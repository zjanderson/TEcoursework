...
//		try {
//
//			x = Integer.parseInt("2"); // if this is here, only the first exception occurs
//
//			try {
//				int[] array = { 1, 2, 3 };
...
//		System.out.println();
	...
	private static double calculateHotelRoomCharges(int nights, int numberOfGuests) {
		...
		if(nights < 1) {
			IllegalArgumentException ex = new IllegalArgumentException("Minimum number of nights is 1");
			throw ex;
	...
	/**
	 * Description of the method here. Withdraw an amount from the current balance. Return the remaining balance.
	 * @param currentBalance current balance for the account
	 * @param amountToWithdraw the amount to withdraw from the current balance
