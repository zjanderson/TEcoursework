...
function allDivisibleByThree(numbersToFilter) {
  const filteredNumbers = numbersToFilter.filter(num => num % 3 === 0);
...
/*
  strings -> ['Three', 'Two', 'Fourteen', 'Five']
 ...
 * 
 * 
 * [ 1, 5, 25.2, 25.256 ] -> [ '$1.00', '$5.00', '$25.20', '$25.26' ]
 */
function toMoney(numbers) {
  const moneyNumbers = [];

  for (num of numbers) {
    const moneyNum = `$${num.toFixed(2)}`;
    moneyNumbers.push(moneyNum);
  }
...
function toMoney(numbers) {
  return numbers.map(translation);
}
...
function exampleOfFilterWith3Params(numbers) {
...
// String myName = "Walt";
// myName.toLowerCase().substring(0, 1).equals("w")